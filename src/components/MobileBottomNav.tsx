...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <nav 
      ...
      className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#e6dfd3] px-2 py-1 shadow-lg safe-area-bottom"
    >
      <div className="flex items-center justify-around">
        {/* Sightings List */}
        <button
          onClick={() => onSelectView('compact_table')}
          className={`flex flex-col items-center justify-center p-1 rounded-lg transition-colors min-w-[56px] cursor-pointer ${
        ...
        >
          ...
          <span className="text-[10px] mt-0.5">Life List</span>
        </button>
        ...
        <button
          ...
          className="flex flex-col items-center justify-center -mt-4 bg-[#2e4a36] text-white p-3 rounded-full shadow-lg hover:bg-[#243b2a] active:scale-95 transition-all border-2 border-white cursor-pointer"
        ...
        <button
          onClick={onOpenMore}
          className={`flex flex-col items-center justify-center p-1 rounded-lg transition-colors min-w-[56px] relative cursor-pointer ${
            isFilterActive ? 'text-[#99582a] font-bold' : 'text-[#6b7568]'
          }`}
        >
