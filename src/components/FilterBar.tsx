...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  return (
    <div className="bg-[#f2ede4] border-b border-[#e6dfd3] px-3 sm:px-6 py-1.5 sm:py-2">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 text-xs">
        ...
        <div className="relative flex-1 max-w-md min-w-[160px]">
          ...
          <input
            ...
            type="text"
            placeholder="Search species, exhibit, tags..."
            value={filter.search}
            onChange={e => setFilter(prev => ({ ...prev, search: e.target.value }))}
            className="w-full bg-white border border-[#d8d0c4] rounded-md pl-8 pr-7 py-1 text-xs text-[#1f241d] placeholder-[#828d7e] focus:outline-none focus:ring-1 focus:ring-[#2e4a36] focus:border-[#2e4a36]"
          />
          {filter.search && (
            <button
              onClick={() => setFilter(prev => ({ ...prev, search: '' }))}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-[#828d7e] hover:text-[#1f241d] cursor-pointer"
            >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden sm:inline-flex rounded-md bg-[#e4dcce] p-0.5 border border-[#d8d0c4]">
            <button
            ...
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-all cursor-pointer ${
                filter.wildStatus === 'wild'
                  ...
                  : 'text-[#2e4a36] hover:bg-[#d8d0c4]/40'
              }`}
            >
              🌿 Wild
            </button>
            <button
              ...
              className={`px-2 py-0.5 rounded text-[11px] font-semibold transition-all cursor-pointer ${
                filter.wildStatus === 'captive'
                  ? 'bg-[#99582a] text-white shadow-2xs'
                  : 'text-[#99582a] hover:bg-[#d8d0c4]/40'
              }`}
            >
              🏛️ Captive
            </button>
          ...
          <div className="relative hidden md:block">
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border transition-colors cursor-pointer ${
                activeFilterCount > 0
                  ? 'bg-[#eef3ed] border-[#2e4a36] text-[#2e4a36]'
            ...
            >
              ...
              <span>Filters</span>
              {activeFilterCount > 0 && (
                <span className="w-4 h-4 rounded-full bg-[#2e4a36] text-white text-[10px] flex items-center justify-center font-mono">
                  {activeFilterCount}
                </span>
              )}
            ...
            {showFilterDropdown && (
              <div className="absolute right-0 mt-1 w-64 bg-white border border-[#d8d0c4] rounded-lg shadow-lg p-3 z-40 space-y-2.5 animate-in fade-in zoom-in-95 duration-100">
                <div className="flex items-center justify-between border-b border-[#f0eae0] pb-1.5">
                  <span className="font-bold text-[#1f241d] text-xs">Filter Life List</span>
                  {isFiltered && (
                    <button
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#f9f8f5] border border-[#d8d0c4] rounded px-2 py-1 text-xs text-[#1f241d]"
                  >
                ...
                <button
                  ...
                  className="w-full mt-2 py-1 bg-[#2e4a36] hover:bg-[#233a2b] text-white font-bold rounded text-xs transition-colors cursor-pointer"
                >
                  Apply
                </button>
              </div>
          ...
          <div className="flex items-center gap-1">
            <ArrowUpDown className="w-3 h-3 text-[#828d7e] hidden xs:block" />
            <select
              id="filter-sort-select"
              value={filter.sortBy}
              onChange={e => setFilter(prev => ({ ...prev, sortBy: e.target.value as any }))}
              className="bg-white border border-[#d8d0c4] rounded-md px-2 py-1 text-xs text-[#1f241d] focus:outline-none focus:ring-1 focus:ring-[#2e4a36]"
            >
              <option value="date_desc">Newest First</option>
              <option value="date_asc">Oldest First</option>
              <option value="vernacular_asc">Common Name</option>
              <option value="scientific_asc">Scientific Binomial</option>
          ...
          {isLifeListView && (
            <div className="hidden sm:inline-flex rounded-md bg-[#e4dcce] p-0.5 border border-[#d8d0c4] ml-1">
              <button
          ...
          {onOpenMobileFilters && (
            <button
              ...
              className="md:hidden flex items-center gap-1 bg-white border border-[#d8d0c4] px-2 py-1 rounded-md text-xs font-semibold text-[#1f241d] cursor-pointer"
            >
              <SlidersHorizontal className="w-3.5 h-3.5 text-[#2e4a36]" />
              {activeFilterCount > 0 && (
                <span className="w-1.5 h-1.5 rounded-full bg-[#99582a]" />
              )}
            </button>
          )}
          ...
          {isFiltered && (
            <button
              ...
              className="hidden sm:inline-block px-1.5 py-0.5 text-[11px] text-[#99582a] hover:bg-[#ebd5c8] rounded font-semibold transition-colors cursor-pointer"
            >
              Clear
            </button>
          )}
